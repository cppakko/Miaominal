...
pub(in crate::ui::shell::layout) fn render_session_snippets_panel(
    ...
    let snippets_filter_input = session_controller
        .read(cx)
        .workspace_snippets_filter_input();
    let filter_text = snippets_filter_input
        .read(cx)
        .value()
        .trim()
        .to_ascii_lowercase();
    let snippets = session_controller.read(cx).snippets().clone();
    ...
    let mut package_summaries: Vec<_> =
        SessionController::collect_available_snippet_packages(&search_matched_snippets)
            .into_iter()
            .map(|package| {
    ...
    let selected_package_filter = session_controller
        .read(cx)
        .workspace_snippets_selected_package_filter();
    let package_filter_controller = session_controller.clone();
    let terminal_controller = session_controller;
    let selected_package_filter = selected_package_filter.as_deref().filter(|selected| {
        package_summaries
            .iter()
    ...
    let content = if snippets.is_empty() {
        ...
        for snippet in visible_snippets {
            let terminal_controller = terminal_controller.clone();
                                ...
                                .child(div().id(button_id).child(icon_button(
                                    ...
                                    move |_window, cx| {
                                        ...
                                        if let Some(tab_id) = terminal_tab_id {
                                            terminal_controller.update(cx, |controller, cx| {
                                                controller.paste_terminal_text(
                                                    tab_id,
                                                    script.clone(),
                                                    cx,
                                                );
            ...
            .when(!package_summaries.is_empty(), |this| {
                this.child(
                    v_flex()
                        ...
                        .child(v_flex().w_full().gap_2().children(
                            package_summaries.into_iter().map(|(package, count)| {
                                let package_name = package.clone();
                                let is_selected = selected_package_filter.is_some_and(|selected| {
                                    selected.eq_ignore_ascii_case(package_name.as_str())
                                });
                                session_snippet_package_card(package, count, is_selected, {
                                    let controller = package_filter_controller.clone();
                                    move |_, cx| {
                                        let package_name = package_name.clone();
                                        controller.update(cx, |controller, cx| {
                                            controller
                                                .handle_workspace_snippets_package_filter_toggle(
                                                    package_name.clone(),
                                                    cx,
                                                );
                                        });
