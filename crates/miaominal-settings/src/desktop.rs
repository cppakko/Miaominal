...
mod tests {
    ...
    #[test]
    fn key_binding_matches_modifiers_and_key_case_insensitively() {
        let binding = KeyBinding::new(true, false, true, "K");

        assert!(binding.matches_keystroke(&keystroke("k", true, false, true, false)));
        assert!(!binding.matches_keystroke(&keystroke("k", true, true, true, false)));
        assert!(!binding.matches_keystroke(&keystroke("k", true, false, true, true)));
    }

    #[test]
    fn interface_font_uses_current_family_and_fallbacks() {
        let configured_fallbacks = font_fallbacks();
        let interface_font = interface_font();

        assert_eq!(interface_font.family.to_string(), font_family());
        match interface_font.fallbacks {
            Some(fallbacks) => {
                assert_eq!(fallbacks.fallback_list(), configured_fallbacks.as_slice())
            }
    ...
    #[test]
    fn desktop_font_catalog_is_never_empty() {
        assert!(!crate::data::available_font_families().is_empty());
    }
}
