...
fn build_remote_sftp_context_menu(
    ...
    if let Some(entry) = context_menu_remote_sftp_entry(&entity, tab_id, cx) {
        ...
        if entry.kind == sftp::SftpEntryKind::Directory {
            let open_entity = entity.clone();
            let open_path = entry.path.clone();
            menu = menu.item(PopupMenuItem::new(i18n::string("sftp.menu.open")).on_click(
                move |_, _, cx| {
        ...
        let is_file = entry.kind != sftp::SftpEntryKind::Directory;
...
impl AppView {
    ...
    pub(in crate::ui::shell) fn render_sftp_page_content(
        ...
        let local_list = div()
            ...
            .when_some(sftp_tab.local_drag_selection.clone(), |this, drag| {
        ...
        let remote_list = div()
            ...
            .when_some(sftp_tab.remote_drag_selection.clone(), |this, drag| {
                let bounds = drag.bounds();
                this.child(
                    div()
                        .absolute()
