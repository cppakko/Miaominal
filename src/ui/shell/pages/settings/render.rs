...
impl SelectItem for AppLanguage {
    type Value = Self;
...
impl SelectItem for MonitorHistoryDuration {
    type Value = Self;

    fn title(&self) -> SharedString {
        SharedString::from(monitor_history_duration_label(*self))
    }
...
impl SelectItem for LocalVaultAutoLockDuration {
    type Value = Self;

    fn title(&self) -> SharedString {
        SharedString::from(local_vault_auto_lock_duration_label(*self))
    }
...
impl SelectItem for LastTabCloseBehavior {
    type Value = Self;

    fn title(&self) -> SharedString {
        SharedString::from(last_tab_close_behavior_label(*self))
    }
...
impl SelectItem for ImportSourceKind {
    type Value = Self;

    fn title(&self) -> SharedString {
        SharedString::from(localized_profile_import_source_label(*self))
    }
...
impl SelectItem for TerminalRightClickBehavior {
    type Value = Self;

    fn title(&self) -> SharedString {
        SharedString::from(match self {
            Self::ContextMenu => i18n::string("settings.key_bindings.context_menu_option"),
            Self::CopySelectionOrPaste => i18n::string("settings.key_bindings.copy_paste_option"),
        })
    }
...
impl SelectItem for SyncProvider {
    type Value = Self;

    fn title(&self) -> SharedString {
        SharedString::from(match self {
            Self::None => i18n::string("settings.sync.providers.none"),
            Self::GithubGist => i18n::string("settings.sync.providers.gist"),
            Self::WebDav => i18n::string("settings.sync.providers.webdav"),
        })
    }
...
fn appearance_page(entity: Entity<AppView>) -> SettingPage {
    ...
    let font_size_min = format!("{:.1}", settings::FONT_SIZE_MIN);
    let font_size_max = format!("{:.1}", settings::FONT_SIZE_MAX);
    let line_height_min = format!("{:.1}", settings::LINE_HEIGHT_MIN);
    let line_height_max = format!("{:.1}", settings::LINE_HEIGHT_MAX);
            ...
            SettingGroup::new()
                ...
                .items(vec![
                    ...
                    .description(i18n::string_args(
                        ...
                        &[("max", &settings::RECENT_CONNECTIONS_COUNT_MAX.to_string())],
                    )),
                ]),
            SettingGroup::new()
            ...
            SettingGroup::new()
                ...
                .item(SettingItem::render(|_, _, _| {
                    ...
                    v_flex()
                        ...
                        .child(
                            div()
                                .text_size(settings::scaled_font_size(16.0))
...
impl SettingFieldElement for SeedColorField {
    ...
    fn render_field(
        ...
        let picker = self
            ...
            .clone();
        let entity = self.entity.clone();
        let material = settings::Theme::from_settings(&current).material;
...
fn render_font_size_stepper(
    ...
    stepper_control(
        ...
        move |_, cx| {
            let entity = entity_for_dec.clone();
            entity.update(cx, |this, cx| {
                this.adjust_font_size(-settings::STEP, cx);
            });
        },
        move |_, cx| {
            let entity = entity_for_inc.clone();
...
fn render_line_height_stepper(
    ...
    stepper_control(
        ...
        move |_, cx| {
            let entity = entity_for_dec.clone();
            entity.update(cx, |this, cx| {
                this.adjust_line_height(-settings::STEP, cx);
            });
        },
        move |_, cx| {
            let entity = entity_for_inc.clone();
