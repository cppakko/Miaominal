use crate::settings::TerminalRightClickBehavior;
...
fn session_snippet_package_card(
    ...
    let icon = crate::domain::snippet::package_initials(&title)
...
fn format_monitor_time_axis_label(total_seconds: usize) -> String {
    ...
    i18n::string_args(
        "workspace.panel.monitor.axis_minutes_seconds_ago",
        &[
            ("minutes", &minutes.to_string()),
...
impl AppView {
        ...
        div()
            ...
            .child(
                v_flex()
                    ...
                    .child(
                        div()
                            .text_size(settings::scaled_font_size(22.0))
                            .text_color(rgb(roles.on_surface))
                                    ...
                                    this.child(icon_button(
                                        ...
                                        None,
                                        {
                                            ...
                                            move |_window, cx| {
                                                weak.update(cx, |this, cx| {
                                                    let Some(tab_index) =
                                                        this.workspace_state.tabs.iter().position(|t| t.id == tab_id)
                                                    else {
                                                        return;
                                                    };
                                                    let profile = this
    ...
    fn render_session_disconnected_banner(
        ...
        render_basic_dialog_with_config(
            ...
            i18n::string("session.workspace.session_closed"),
            supporting_text,
            Some(body),
            actions,
            Some(BasicDialogIcon {
                icon: AppIcon::Minimize,
                tint: roles.on_surface_variant,
            }),
            BasicDialogHeaderAlignment::Center,
            None,
        )
    }
                    ...
                    div()
                        ...
                        .child(
                            v_flex()
                                ...
                                .child(
                                    div()
                                        .text_size(settings::scaled_font_size(18.0))
                                        .text_color(rgb(roles.on_surface))
    ...
    fn render_session_monitor_panel(
        ...
        let content =
            if !monitoring.auto_collect_enabled {
                ...
                v_flex()
                    .gap_2()
                    .child(self.render_monitor_chart_card(
                        ...
                        cpu_y_max,
                        cpu_y_ticks.clone(),
                        build_chart_tick_labels(&cpu_y_ticks, format_percentage),
                        0,
                    ))
                    .child(
                        h_flex()
                            ...
                            .child(div().flex_1().min_w(px(0.0)).child(
                                self.render_monitor_chart_card(
                                    i18n::string("workspace.panel.monitor.memory"),
                                    format_percentage(snapshot.memory_percent),
                                    None,
                                    &memory_history,
                                    memory_y_max,
                                    memory_y_ticks.clone(),
                                    build_chart_tick_labels(&memory_y_ticks, format_percentage),
                                    1,
                                ),
                            ))
                            .child(div().flex_1().min_w(px(0.0)).child(
                                self.render_monitor_chart_card(
                                    i18n::string("workspace.panel.monitor.swap"),
                                    format_percentage(snapshot.swap_percent),
                                    None,
                                    &swap_history,
                                    swap_y_max,
                                    swap_y_ticks.clone(),
                                    build_chart_tick_labels(&swap_y_ticks, format_percentage),
                                    2,
                                ),
                            )),
                    )
                    .child(self.render_monitor_chart_card(
                        i18n::string("workspace.panel.monitor.network"),
                        format_rate_label(snapshot.network_rx_kbps + snapshot.network_tx_kbps),
                        ...
                        &network_history,
                        network_y_max,
                        network_y_ticks.clone(),
                        build_chart_tick_labels(&network_y_ticks, format_rate_label),
                        3,
                    ))
                    .child(self.render_monitor_chart_card(
                        i18n::string("workspace.panel.monitor.load"),
                        format_load_label(snapshot.load),
                        None,
                        &load_history,
                        load_y_max,
                        load_y_ticks.clone(),
                        build_chart_tick_labels(&load_y_ticks, format_load_label),
                        4,
                    ))
                    .into_any_element()
            } else {
        ...
        card_surface(roles.surface_container, 16.0)
            ...
            .overflow_hidden()
            .child(
                v_flex().size_full().overflow_hidden().p_3().child(
                    div()
                        .flex_1()
                        .min_h(px(0.0))
    ...
    fn render_monitor_chart_card(
        &self,
        title: String,
        value: String,
        detail: Option<String>,
        history: &[MonitorChartPoint],
        y_max: f64,
        y_ticks: Vec<f64>,
        ...
        let axis_label_font_size = settings::scaled_font_size(8.0);
        ...
        let time_axis_labels =
            (!chart_data.is_empty()).then(|| build_monitor_time_axis_labels(chart_data.len()));
            ...
            .child(
                h_flex()
                    ...
                    .when_some(detail, |this, detail| {
                        this.child(
                            div()
                                .text_size(settings::scaled_font_size(9.0))
                                .text_color(rgb(text_muted))
                                .child(detail),
                        )
                    }),
    ...
    fn render_session_snippets_panel(&self, entity: Entity<Self>, cx: &App) -> gpui::AnyElement {
        ...
        let search_matched_snippets: Vec<_> = self
            ...
            .snippets
            .iter()
            .filter(|snippet| crate::domain::snippet::matches_filter(snippet, &filter_text))
            .cloned()
            .collect();
