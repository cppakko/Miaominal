use super::*;
use crate::domain::profile::{DEFAULT_SESSION_CHARSET, ImportSourceKind};
...
impl AppView {
    fn font_family_options(current_font_family: &str) -> Vec<String> {
        let mut families = settings::available_font_families();
        let default_font_family = settings::default_font_family();
        if !families
            .iter()
    ...
    pub fn new(runtime: TokioHandle, window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let language_options = AppLanguage::supported_languages().to_vec();
        let selected_language_index = language_options
            .iter()
            .position(|language| *language == settings_store.settings().language)
        ...
        let language_select_subscription = cx.subscribe(
            &language_select,
            |this: &mut AppView, _, event: &SelectEvent<Vec<AppLanguage>>, cx| {
        ...
        let last_tab_close_behavior_options = settings::LastTabCloseBehavior::all().to_vec();
        let selected_last_tab_close_behavior_index = last_tab_close_behavior_options
            .iter()
            .position(|behavior| *behavior == settings_store.settings().last_tab_close_behavior)
            .map(|index| IndexPath::default().row(index));
        ...
        let last_tab_close_behavior_select_subscription = cx.subscribe(
            ...
            |this: &mut AppView,
             _,
             event: &SelectEvent<Vec<settings::LastTabCloseBehavior>>,
             cx| {
        ...
        let local_vault_auto_lock_duration_options =
            settings::LocalVaultAutoLockDuration::all().to_vec();
        let selected_local_vault_auto_lock_duration_index = local_vault_auto_lock_duration_options
            .iter()
            .position(|duration| {
                *duration == settings_store.settings().local_vault_auto_lock_duration
            })
            .map(|index| IndexPath::default().row(index));
        ...
        let local_vault_auto_lock_duration_select_subscription = cx.subscribe(
            ...
            |this: &mut AppView,
             _,
             event: &SelectEvent<Vec<settings::LocalVaultAutoLockDuration>>,
             cx| {
        ...
        let monitor_history_options = settings::MonitorHistoryDuration::all().to_vec();
        let selected_monitor_history_index = monitor_history_options
            .iter()
            .position(|d| *d == settings_store.settings().monitor_history_duration)
        ...
        let monitor_history_select_subscription = cx.subscribe(
            ...
            |this: &mut AppView,
             _,
             event: &SelectEvent<Vec<settings::MonitorHistoryDuration>>,
             cx| {
                let SelectEvent::Confirm(selected) = event;
                if let Some(duration) = selected {
        ...
        let terminal_right_click_behavior_options = vec![
            settings::TerminalRightClickBehavior::ContextMenu,
            settings::TerminalRightClickBehavior::CopySelectionOrPaste,
        ];
        let selected_terminal_right_click_behavior_index = terminal_right_click_behavior_options
            .iter()
            .position(|behavior| {
                *behavior == settings_store.settings().terminal_right_click_behavior
            })
            .map(|index| IndexPath::default().row(index));
        ...
        let terminal_right_click_behavior_select_subscription = cx.subscribe(
            ...
            |this: &mut AppView,
             _,
             event: &SelectEvent<Vec<settings::TerminalRightClickBehavior>>,
             cx| {
                let SelectEvent::Confirm(selected_behavior) = event;
                if let Some(behavior) = selected_behavior {
        ...
        let selected_sync_provider_index = sync_provider_options
            .iter()
            .position(|provider| *provider == sync_engine.config_store.config.provider)
            .map(|index| IndexPath::default().row(index));
        ...
        let sync_provider_select_subscription = cx.subscribe(
            ...
            |this: &mut AppView, _, event: &SelectEvent<Vec<SyncProvider>>, cx| {
        ...
        let seed_color = settings::Theme::from_settings(settings_store.settings())
            .material
            .source;
        ...
        let host_editor_forms = Self::build_host_editor_forms(
            name_input,
            group_input,
            group_select,
            managed_key_select,
            proxy_jump_select,
            charset_select,
            !selected_group.is_empty() && selected_existing_group.is_none(),
            tags_input,
            host_input,
            port_input,
            username_input,
        ...
        let workspace_forms = Self::build_workspace_forms(
            rename_input,
            search_input,
            session_snippets_filter_input,
            local_sftp_path_input,
        ...
        let panel_forms = Self::build_panel_forms(
            filter_input,
            keychain_filter_input,
        ...
        let mut view = Self {
            ...
            _subscriptions: Self::build_subscriptions(
