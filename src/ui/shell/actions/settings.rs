...
use super::super::*;
use crate::domain::sync::{SyncConfig, SyncProvider, SyncStatus};
...
impl AppView {
    ...
    pub(in crate::ui::shell) fn reset_font_family(
        ...
        let default_font = settings::default_font_family();
        let changed = self
            .settings_store
    ...
    pub(in crate::ui::shell) fn reset_font_fallbacks(
        ...
        let defaults = settings::default_font_fallbacks();
    ...
    pub(in crate::ui::shell) fn reset_seed_color(
        ...
        let default_color = settings::Theme::from_settings(self.settings_store.settings())
            .material
            .source;
        self.panel_forms
            .settings
    ...
    pub(in crate::ui::shell) fn adjust_recent_connections_count(
        ...
        let next = (current + delta).clamp(
            settings::RECENT_CONNECTIONS_COUNT_MIN as i16,
            settings::RECENT_CONNECTIONS_COUNT_MAX as i16,
        ) as u8;
        let changed = self
            .settings_store
    ...
    pub(in crate::ui::shell) fn reset_key_binding(
        ...
        use crate::settings::TerminalKeyBindings;
    ...
    fn spawn_local_vault_unlock(&mut self, passphrase: String, cx: &mut Context<Self>) {
        ...
        if let Err(error) = spawn_result {
            ...
            if let Some(window_handle) = notification_window.as_ref() {
                if let Err(update_error) = window_handle.update(cx, move |_, window, cx| {
                    window.push_notification(
                        Self::error_notification(
                            i18n::string("settings.sync.vault.notifications.failed_title"),
                            message,
        ...
        cx.spawn(async move |this, cx| {
            let result = cx
                ...
                .spawn(async move {
                    rx.recv()
                        .unwrap_or_else(|_| Err(anyhow::anyhow!("local vault unlock task cancelled")))
                })
                .await;
            ...
            if let Some(window_handle) = notification_window {
                ...
                let update_result = window_handle.update(cx, move |_, window, cx| {
                    ...
                    if let Err(error) = this_for_window.update(cx, move |this, cx| {
                        ...
                        log::debug!("failed to apply local vault unlock result in window: {error:?}");
                    }
                });
            ...
            if let Some(window_handle) = notification_window {
                ...
                let update_result = window_handle.update(cx, move |_, window, cx| {
                    ...
                    if let Err(error) = this_for_window.update(cx, move |this, cx| {
                        ...
                        log::debug!("failed to apply local vault disable result in window: {error:?}");
                    }
                });

                if let Err(error) = update_result {
                    log::debug!("failed to access active window for local vault disable: {error:?}");
                    if let Some(result) = result.borrow_mut().take() {
                        if let Err(error) = this.update(cx, move |this, cx| {
    ...
    fn spawn_local_vault_enable(&mut self, passphrase: String, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |this, cx| {
            let result = cx
                ...
                .spawn(async move {
                    rx.recv()
                        .unwrap_or_else(|_| Err(anyhow::anyhow!("local vault enable task cancelled")))
                })
                .await;
            ...
            if let Some(window_handle) = notification_window {
                ...
                let update_result = window_handle.update(cx, move |_, window, cx| {
                    ...
                    if let Err(error) = this_for_window.update(cx, move |this, cx| {
                        ...
                        log::debug!("failed to apply local vault enable result in window: {error:?}");
                    }
                });
