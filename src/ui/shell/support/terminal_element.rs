use crate::settings;
use crate::terminal::{SearchMatchKind, TerminalSnapshot, terminal_font, terminal_font_size};
...
mod tests {
    use super::*;
    use crate::terminal::{default_background, default_foreground};
    ...
    fn hovered_link_range_stays_on_hovered_run_only() {
        ...
        let mut cells = "a link b link"
            .chars()
            .map(|character| {
                let mut cell = crate::terminal::TerminalCell::blank(fg, bg);
        ...
        for index in 2..6 {
            cells[index].link = Some("https://example.test".into());
        }
        for index in 9..13 {
            cells[index].link = Some("https://example.test".into());
        }
