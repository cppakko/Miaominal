...
pub(in crate::ui::shell) struct SftpBrowserTableRow {
    ...
    pub(in crate::ui::shell) kind: sftp::SftpEntryKind,
...
impl SftpBrowserTableRow {
    pub(in crate::ui::shell) fn from_local(entry: &LocalSftpEntry) -> Self {
        let kind = if entry.is_directory {
            sftp::SftpEntryKind::Directory
        } else {
            sftp::SftpEntryKind::File
        };
    ...
    pub(in crate::ui::shell) fn from_remote(entry: &SftpEntry) -> Self {
        ...
        Self {
            ...
            name: entry.filename.clone().into(),
            is_directory: entry.kind == sftp::SftpEntryKind::Directory,
            kind: entry.kind,
            size: entry.size,
            type_label,
            modified: entry.modified,
    ...
    fn localized_type_label(side: SftpBrowserSide, kind: sftp::SftpEntryKind) -> String {
        match (side, kind) {
            (SftpBrowserSide::Local, sftp::SftpEntryKind::Directory) => {
                i18n::string("sftp_browser.types.folder")
            }
            (_, sftp::SftpEntryKind::Directory) => i18n::string("sftp_browser.types.directory"),
            (_, sftp::SftpEntryKind::Symlink) => i18n::string("sftp_browser.types.symlink"),
            (_, sftp::SftpEntryKind::Other) => i18n::string("sftp_browser.types.other"),
            (_, sftp::SftpEntryKind::File) => i18n::string("sftp_browser.types.file"),
        }
    }
}
    ...
    fn flatten_into_visible(&mut self, entries: &[SftpBrowserTableRow], depth: usize) {
        ...
        for mut row in sorted {
            ...
            if is_directory && is_expanded {
                if let Some(children) = self.children_source.get(&path).cloned() {
    ...
    fn render_td(
        ...
        match orig_col_ix {
            0 => {
                ...
                if is_renaming {
                    if let Some(inline_rename_input) = self.app_view.upgrade().map(|view| {
                        view.read(cx)
                            .workspace_forms
                            .sftp_browser
                            .inline_rename_input
