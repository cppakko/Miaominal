use crate::domain::keychain::{ManagedKeyRecord, ManagedKeySource};
use crate::domain::profile::{AuthMethod, SessionProfile, ShellType};
use crate::infra::keychain_store::ManagedKeyStore;
use crate::infra::known_hosts_store::KnownHostsStore;
use crate::infra::ssh::{self, AgentIdentitySummary};
use crate::secrets::{SecretKind, SecretStore};
use anyhow::Result;
use tokio::runtime::Handle as TokioHandle;

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct ImportedManagedKey {
    pub record: ManagedKeyRecord,
    pub normalized_private_key: String,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct DeleteManagedKeyOutcome {
    pub removed: ManagedKeyRecord,
    pub cleared_profile_ids: Vec<String>,
}

#[derive(Debug, Clone)]
pub(crate) struct KeychainRefreshData {
...
#[derive(Clone)]
pub(crate) struct KeychainService {
    runtime: TokioHandle,
...
impl KeychainService {
    ...
    pub(crate) fn refresh_data(&self) -> Result<KeychainRefreshData> {
    ...
    pub(crate) fn generate_material(&self) -> Result<(String, String)> {
        Self::generate_ed25519_material()
    }

    pub(crate) fn generate_ed25519_material() -> Result<(String, String)> {
    ...
    pub(crate) fn persist_keys(&self, keys: &[ManagedKeyRecord]) -> Result<()> {
    ...
    pub(crate) fn profile_supports_deploy(profile: &SessionProfile) -> bool {
