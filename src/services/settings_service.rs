...
use crate::infra::sync::{credential_migration, engine::SyncEngine};
use crate::secrets::{SecretStore, VaultCredentialBackend};
use crate::settings::{
    FONT_SIZE_MAX, FONT_SIZE_MIN, LINE_HEIGHT_MAX, LINE_HEIGHT_MIN, SettingsStore,
};
...
#[derive(Clone)]
pub(crate) struct LocalVaultTransition {
...
pub(crate) enum LocalVaultPassphraseChangeOutcome {
...
impl SettingsService {
    pub(crate) fn adjust_font_size(settings_store: &mut SettingsStore, delta: f32) -> Option<f32> {
        let target =
    ...
    pub(crate) fn adjust_line_height(
    ...
    pub(crate) fn set_sync_provider(
    ...
    pub(crate) fn persist_sync_github_token(
        sync_engine: &mut SyncEngine,
        token: &str,
    ) -> Result<()> {
    ...
    pub(crate) fn persist_sync_webdav_password(
        sync_engine: &mut SyncEngine,
        password: &str,
    ) -> Result<()> {
    ...
    pub(crate) fn persist_sync_passphrase(
        sync_engine: &mut SyncEngine,
    ...
    pub(crate) fn local_vault_lock_transition(
    ...
    pub(crate) fn prepare_vault_enable(
        passphrase: &str,
        ...
        if copy_result.is_err() {
            if let Err(erase_err) = VaultCredentialBackend::erase_default_store_file() {
                log::warn!(
                    "failed to erase partial vault file after enable failure: {erase_err:?}"
                );
            }
        }
    ...
    pub(crate) fn apply_vault_enable(
        passphrase: String,
    ...
    pub(crate) fn unlock_local_vault(passphrase: String) -> Result<LocalVaultTransition> {
    ...
    pub(crate) fn change_local_vault_passphrase(
    ...
    pub(crate) fn prepare_vault_disable(
    ...
    pub(crate) fn apply_vault_disable(settings_store: &mut SettingsStore) -> Result<()> {
        let mut settings = settings_store.settings().clone();
    ...
    pub(crate) fn delete_migrated_keyring_secrets(
        session_ids: &[String],
        managed_key_ids: &[String],
        source_secrets: &SecretStore,
        source_sync_engine: &SyncEngine,
    ...
    pub(crate) fn erase_vault_file() -> Result<()> {
...
mod tests {
    ...
    use crate::infra::credential_backend::{APP_CREDENTIAL_SERVICE, CredentialStore};
    use crate::infra::sync::store::SyncConfigStore;
    use crate::secrets::{SecretKind, SecretStore, VaultCredentialBackend};
    use std::fs;
