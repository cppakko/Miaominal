...
use crate::domain::known_host::HostKeyCheck;
use crate::domain::profile::SessionProfile;
use crate::domain::sftp::{SftpEntry, TransferDirection, TransferId};
...
async fn run_session(
    ...
    if let Ok(initial_path) = sftp.canonicalize(".").await {
        if let Ok(entries) = list_directory_entries(&sftp, &initial_path).await {
