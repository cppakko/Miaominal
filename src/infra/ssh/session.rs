...
use crate::domain::forwarding::{
    HostKeyDecision, HostKeyPrompt, KbiChallenge, SessionMonitorSnapshot,
};
use crate::domain::known_host::{HostKeyCheck, algorithm_of, fingerprint_of};
...
use crate::terminal::MIN_TERMINAL_COLUMNS;
...
pub(crate) mod connection {
    ...
    pub(crate) fn default_client_config() -> Arc<client::Config> {
    ...
    pub(crate) fn resolve_proxy_jump_profiles(
    ...
    pub(crate) async fn connect_profile_session<H>(
        profile: &SessionProfile,
    ...
    pub(crate) async fn connect_profile_stream<H, R>(
        profile: &SessionProfile,
        transport: R,
        config: Arc<client::Config>,
        handler: H,
