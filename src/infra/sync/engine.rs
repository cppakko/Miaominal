...
use crate::domain::sync::{SyncPayload, SyncProvider, SyncStatus};
...
impl SyncEngine {
    ...
    async fn push_internal(
        ...
        if !force {
            if let RemotePayloadState::Newer(payload) = self.remote_payload_state().await? {
    ...
    pub(crate) fn sync_enabled_for_provider(&self) -> bool {
