...
pub use crate::domain::terminal::{SearchMatchKind, TerminalInputModes, sanitize_paste};
use crate::settings;
...
impl TerminalCell {
    pub(crate) fn blank(fg: Hsla, bg: Hsla) -> Self {
        Self {
            character: ' ',
            zero_width: Vec::new(),
            fg,
        ...
        for (idx, range) in search.matches.iter().enumerate() {
            ...
            loop {
                ...
                if let Ok(line_index) = usize::try_from(viewport_line) {
                    if line_index < screen_lines && current.column.0 < columns {
